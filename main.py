...
class P2P_Node:
    ...
    def rece(self):
        while 1:
            data, addr = pu.receber_mensagem_base(self.udp_socket)
        ...
        # def dispatch(self, action,addr):
            if action['type'] == 'newpeer':
                ...
                # print(addr)
                pu.enviar_mensagem_JSON(self.udp_socket, addr,{
                "type":'peers',
                "data":self.peers
                })         
            ...
            if action['type'] == 'peers':
                ...
                # introduce youself. 
                pu.broadcast_mensagem_JSON(self.udp_socket, {
                    "type":"introduce",
                    "data": self.myid
                },self.peers) 
    ...
    def startpeer(self):
        pu.enviar_mensagem_JSON(self.udp_socket,self.seed,{
            "type":"newpeer",
            "data":self.myid
        })
    ...
    def send(self):
        while 1: 
            ...
            if l[-1] in self.peers.keys():
                toA = self.peers[l[-1]]
                s = ' '.join(l[:-1]) 
                pu.enviar_mensagem_JSON(self.udp_socket, toA,{
                    "type":"input",
                    "data":s
                })      
            else :
